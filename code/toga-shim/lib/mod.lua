-- TOGA-SHIM — system-wide Toga Grid/Arc with practical, low-clutter menu
-- • Per-device force: Grid + Arc each Auto / Always / Off
-- • Save/auto-load TouchOSC clients
-- • Test lights for quick diagnostics
...
-- Force modes: 0 = AUTO (use Toga only if no physical), 1 = ALWAYS, 2 = OFF
local FORCE_GRID = 0
local FORCE_ARC  = 0
...
local function save_prefs()
  ...
  if not ok then
    ...
    if f then
      f:write(string.format("return { FORCE_GRID=%d, FORCE_ARC=%d }\n", FORCE_GRID, FORCE_ARC))
      f:close()
    end
  end
end
...
local function save_last_client_from(tg_or_ta)
  ensure_dir(DATA_DIR)
  local list = util.file_exists(CLIENTS_FN) and (tabutil.load(CLIENTS_FN) or {}) or {}
  if tg_or_ta and tg_or_ta.dest and #tg_or_ta.dest > 0 then
    local last = tg_or_ta.dest[#tg_or_ta.dest]
    local exists=false
...
local function force_label(n)
  return (n==1) and "Always" or (n==2) and "Off" or "Auto"
end

-- ── detection/helpers ───────────────────────────────────────────────────────
local function toga_installed()
  return util.file_exists(_path.code.."toga/lib/togagrid.lua") or util.file_exists(_path.code.."toga/lib/togaarc.lua")
end
...
-- Load prefs immediately; clients are loaded later at system_post_startup
load_prefs()

-- ── pre-init hook: swap grid/arc if needed ───────────────────────────────────
mod.hook.register("script_pre_init", "toga-shim preinit", function()
  if not toga_installed then return end
...
-- Auto-load saved clients after norns boots
mod.hook.register("system_post_startup", "toga-shim load clients", function()
  if toga_installed() then load_saved_clients() end
end)

-- ── menu (compact; K1/K2 exit via mod.menu.exit()) ──────────────────────────
local menu = { i = 1 }

local rows = {
  { name="Grid force", value=function() return force_label(FORCE_GRID) end, action=function()
      FORCE_GRID = (FORCE_GRID+1)%3; save_prefs()
    end },
  { name="Arc force",  value=function() return force_label(FORCE_ARC)  end, action=function()
      FORCE_ARC  = (FORCE_ARC +1)%3; save_prefs()
    end },

  { name="Add current client", value=function() return "" end, action=function()
      -- prefer grid shim as source; fall back to arc
      local okg, tg = pcall(function() return (include "toga/lib/togagrid").connect() end)
      if okg and tg and tg.dest and #tg.dest>0 then
        save_last_client_from(tg)
      else
        local oka, ta = pcall(function() return (include "toga/lib/togaarc").connect() end)
        if oka and ta then save_last_client_from(ta) else print("[toga-shim] no client to add") end
      end
    end },

  { name="Clear clients", value=function() return "" end, action=function()
      clear_saved_clients()
      print("[toga-shim] cleared saved clients")
    end },
  ...
  { name="Test lights", value=function() return "" end, action=function()
      ...
      if okg and tg then
        tg:all(0)
        for y=1,8 do for x=1,16 do tg:led(x,y, ((x+y)%2==0) and 10 or 2) end end
        tg:refresh(true)
      end
      -- arc quarter sweep
      local oka, ta = pcall(function() return (include "toga/lib/togaarc").connect() end)
      if oka and ta then
        ...
        if ta.segment then
          ...
          clock.run(function() clock.sleep(0.6); if ta.all then ta:all(0); if ta.refresh then ta:refresh() end end) end)
        end
      end
